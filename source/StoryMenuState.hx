...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		txtWeekTitle = new FlxText(FlxG.width / 2, 10, 0, "", 32);
		...
		var bgYellow:FlxSprite = new FlxSprite(-(FlxG.width / 2), 0).makeGraphic(2388, 720, 0xFF32f2d9);
		bgSprite = new FlxSprite(-(FlxG.width / 2), 0);
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			var weekThing:MenuItem = new MenuItem(0, FlxG.height / 2, WeekData.weeksList[i]);
			weekThing.y += ((FlxG.height / 2) * i);
			weekThing.targetY = i;
		...
		leftArrow = new FlxSprite(((FlxG.width / 2) - 24) - 250, grpWeekText.members[0].y + 20);
		...
		for (i in 0...CoolUtil.difficultyStuff.length) {
			var sprDifficulty:FlxSprite = new FlxSprite(leftArrow.x + 90, leftArrow.y + 100).loadGraphic(Paths.image('menudifficulties/' + CoolUtil.difficultyStuff[i][0].toLowerCase()));
		...
		rightArrow = new FlxSprite(((FlxG.width / 2) - 24) + 250, leftArrow.y);
		...
		txtTracklist = new FlxText(FlxG.width / 2, (FlxG.height / 2) - 114, 0, "", 32);
	...
	function changeDifficulty(change:Int = 0):Void
	{
		...
		sprDifficultyGroup.forEach(function(spr:FlxSprite) {
			...
			if(curDifficulty == spr.ID) {
				...
				FlxTween.tween(spr, {y: leftArrow.y + 100, alpha: 1}, 0.07);
			}
		});
